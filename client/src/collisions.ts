...
const collideLineCircle = (
  ...
  const inside1 = collidePointCircle(x1, y1, cx, cy, diameter, p);
  const inside2 = collidePointCircle(x2, y2, cx, cy, diameter, p);
  if (inside1 || inside2) return true;

  // get length of the line
  let distX = x1 - x2;
  let distY = y1 - y2;
  const len = p.sqrt(distX * distX + distY * distY);

  // get dot product of the line and circle
  const dot = ((cx - x1) * (x2 - x1) + (cy - y1) * (y2 - y1)) / p.pow(len, 2);
  ...
  const onSegment = collidePointLine(closestX, closestY, x1, y1, x2, y2, p);
